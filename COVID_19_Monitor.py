...
def data():
    ...
    if Visual == "Show Data":
        ...
        R0=10**(m*6)
        DYVals=yVals[-14:]
        DxVals = np.arange(len(DYVals))
        DA=np.vstack([DxVals,np.ones(len(DxVals))]).T
        Dm,Dc=np.linalg.lstsq(DA,DYVals,rcond=None)[0]
        DR0=10**(Dm*6)
...
#        slope=dy/dx
#        R0=10**(slope[0]*6)
        fig.suptitle(str(country)+' ,R0='+str(R0)+" ,Daily R0="+str(DR0))
        plt.xlabel("Number of days since 1/1/2020")
        plt.ylabel("log[n(t)]")
